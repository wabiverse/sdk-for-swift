@_exported import AppwriteServerModels
...
open class Client {
  ...
  open var endPointRealtime: String?
  ...
  private static func createHTTP(
    ...
    maxRedirects _: Int = 5,
    alloweRedirectCycles _: Bool = false,
  ...
  open func setEndpoint(_ endPoint: String) -> Client {
    ...
    if endPointRealtime == nil && endPoint.starts(with: "http") {
  ...
  private func addOriginHeader() {
    ...
    _ = addHeader(
      key: "origin",
      value: "AppwriteServer-\(operatingSystem)://\(packageInfo.packageName)"
    )
  }
}
