...
open class Users: Service {
  ...
  open func list<T>(
    ...
    search: String? = nil,
    nestedType _: T.Type
  ) async throws -> AppwriteServerModels.UserList<T> {
    let path = "/users"
    ...
    let converter: (Any) -> AppwriteServerModels.UserList<T> = { response in
      AppwriteServerModels.UserList.from(map: response as! [String: Any])
    }
  ...
  open func list(
    queries: [String]? = nil,
    search: String? = nil
  ) async throws -> AppwriteServerModels.UserList<[String: AnyCodable]> {
